...
import { Sliders, Sun, Eye, Undo2, RotateCw, ChevronDown, ChevronRight } from 'lucide-react';
  ...
  return (
    <MangaPanel className="space-y-5 text-black">
      ...
      <div className="flex items-center justify-between pb-2 border-b-3 border-black">
        ...
        >
          <Sliders size={18} className="text-[#FF003C] stroke-[2.5] group-hover:scale-110 transition-transform" />
          <h4 className="text-sm font-display font-black text-black uppercase tracking-wider manga-slanted">
            Screentone Ink Adjustments
          </h4>
          {isOpen ? (
            <ChevronDown size={16} className="text-black stroke-[2.5] shrink-0" />
          ) : (
            <ChevronRight size={16} className="text-zinc-500 group-hover:text-black stroke-[2.5] shrink-0" />
          )}
        </button>
        {isModified && (
          <button
